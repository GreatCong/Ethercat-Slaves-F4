 ...
 * @date 2018-6-1
 * @brief _STM32_IO8=1
         ---> 下面是LAN9252的硬件配置，移植从这里修改
         --->  另外，ecatappl.c 里面有对定时器的宏定义,global.c interrupt handle
 *************************************************************************************
*/

#define EC_USE_HAL_lib 1 //定义HAL库
...
-----------------------------------------------------------------------------------------*/
#if AL_EVENT_ENABLED
#define    INIT_ESC_INT           EXTI3_Configuration();	//HAL库没有用到				
#define    EcatIsr                EcatIsr//EXTI3_IRQHandler //HAL库没有用到
...
#define    ACK_SYNC0_INT                   __HAL_GPIO_EXTI_CLEAR_FLAG(SYNC0_Pin) //EXTI_ClearITPendingBit(EXTI_Line1);
#define    IS_SYNC0_INT_ACTIVE             ((HAL_GPIO_ReadPin(SYNC0_GPIO_Port,SYNC0_Pin))==0)//((GPIO_ReadInputDataBit(GPIOC,SYNC0_Pin)) == 0) 
...
#define    ACK_SYNC1_INT                   __HAL_GPIO_EXTI_CLEAR_FLAG(SYNC1_Pin)//EXTI_ClearITPendingBit(EXTI_Line2);
#define    IS_SYNC1_INT_ACTIVE             ((HAL_GPIO_ReadPin(SYNC1_GPIO_Port,SYNC1_Pin))==0)//(GPIO_ReadInputDataBit(GPIOC,SYNC1_Pin)) == 0)  
...
#define ECAT_TIMER_ACK_INT        		 	__HAL_TIM_CLEAR_IT(&htim5, TIM_IT_UPDATE);//TIM_ClearITPendingBit(TIM2 , TIM_FLAG_Update);	
...
#define STOP_ECAT_TIMER              	HAL_TIM_Base_Stop_IT(&htim5)//TIM_Cmd(TIM2, DISABLE);	

#define START_ECAT_TIMER           		HAL_TIM_Base_Start_IT(&htim5) //TIM_Cmd(TIM2, ENABLE);				
...
/*******************************************************************************
  ...
  *****************************************************************************/
//从这一行,下面的函数在HAL库下不编译
#ifndef EC_USE_HAL_lib //HAL lib采用回调函数，不直接调用
void  EcatIsr(void)
{		 
...
#ifndef EC_USE_HAL_lib //HAL lib采用回调函数，不直接调用 回调在global.c中
