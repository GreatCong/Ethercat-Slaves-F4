    ...
    for(AxisCnt = 0; AxisCnt < MAX_AXES ; AxisCnt++)
    {
        ...
        *************************************/
        ...
        for(j =0; j < LocalAxes[AxisCnt].Objects.sRxPDOMap3.u16SubIndex0;j++)
            LocalAxes[AxisCnt].Objects.sRxPDOMap3.aEntries[j] += AxisCnt* (ObjectOffset<<16);
						
				/*ptm RxPDO*/
        for(j =0; j < LocalAxes[AxisCnt].Objects.sRxPDOMap4.u16SubIndex0;j++)
            LocalAxes[AxisCnt].Objects.sRxPDOMap4.aEntries[j] += AxisCnt* (ObjectOffset<<16);
        ...
        for(j =0; j < LocalAxes[AxisCnt].Objects.sTxPDOMap3.u16SubIndex0;j++)
            LocalAxes[AxisCnt].Objects.sTxPDOMap3.aEntries[j] += AxisCnt* (ObjectOffset<<16);
						
				/*ptm TxPDO*/
        for(j =0; j < LocalAxes[AxisCnt].Objects.sTxPDOMap4.u16SubIndex0;j++)
            LocalAxes[AxisCnt].Objects.sTxPDOMap4.aEntries[j] += AxisCnt* (ObjectOffset<<16);
        ...
        *************************************/
        ...
        while(pDiCEntry->Index != 0xFFFF)
        {
            ...
            case 0x1602:
                ...
                break;
						case 0x1604:
                pDiCEntry->pVarPtr = &LocalAxes[AxisCnt].Objects.sRxPDOMap4;
                break;
            case 0x1A00:
            ...
            case 0x1A02:
                ...
                break;
						case 0x1A04:
                pDiCEntry->pVarPtr = &LocalAxes[AxisCnt].Objects.sTxPDOMap4;
                break;
            case 0x603F:
            ...
            case 0x6085:
                ...
                break;
						case 0x6087:
                pDiCEntry->pVarPtr = &LocalAxes[AxisCnt].Objects.objTorqueSlope;
                break;
            case 0x60C2:
            ...
            || pCiA402Axis->Objects.objPositionActualValue < pCiA402Axis->Objects.objTargetPosition))
        {
            ...
            switch(pCiA402Axis->Objects.objModesOfOperationDisplay)
            {
							case CYCLIC_SYNC_POSITION_MODE:
									if(IncFactor != 0)
											i32TargetVelocity = (pCiA402Axis->Objects.objTargetPosition - pCiA402Axis->Objects.objPositionActualValue) / ((float)IncFactor);
									else
											i32TargetVelocity = 0;
									break;
...
extern CMCT oMCT[MC_NUM];  
extern State_t state;

extern int32_t Real_Position ;
...
void CiA402_MotionControl(TCiA402Axis *pCiA402Axis)
{
    ...
    !pCiA402Axis->bBrakeApplied)
    {
            ...
            switch(pCiA402Axis->Objects.objModesOfOperationDisplay)
            {
							...
							case PROFILE_VELOCITY_MODE:
									if(pCiA402Axis->i16State == STATE_OPERATION_ENABLED)
									{
										...
										MCI_ExecSpeedRamp(oMCI[0], (float)i32TargetVelocity/6, 0);	
		...
		pCiA402Axis->Objects.objPositionActualValue = Real_Position;
		pCiA402Axis->Objects.objModesOfOperationDisplay = pCiA402Axis->Objects.objModesOfOperation;
		pCiA402Axis->Objects.objVelocityActualValue = MCI_GetAvrgMecSpeed01Hz(oMCI[0])*6;
}
...
#endif
        {
            ...
            else
            {
                ...
                && (sConfiguredModuleIdentList.aEntries[(i-1)] != (UINT32)CSV_MODULE_ID)
								&& (sConfiguredModuleIdentList.aEntries[(i-1)] != (UINT32)PVM_MODULE_ID)
								&& (sConfiguredModuleIdentList.aEntries[(i-1)] != (UINT32)PTM_MODULE_ID)
        ...
        for (cnt = 0 ; cnt < sConfiguredModuleIdentList.u16SubIndex0; cnt++)
        {
            ...
            switch(sConfiguredModuleIdentList.aEntries[cnt])
            {
                ...
                break;
								...
								case PTM_MODULE_ID:
                    sRxPDOassign.aEntries[cnt] = (0x1604 +(0x10*cnt));
                    sTxPDOassign.aEntries[cnt] = (0x1A04 +(0x10*cnt));
                break;
                default:
        ...
        * => update object 0xF010 (Module profile list) and 0xF050 (Detected Module List)*/
        ...
        for (cnt = 0 ; cnt < sRxPDOassign.u16SubIndex0; cnt++)
        {
            ...
            switch((sRxPDOassign.aEntries[cnt] & 0x000F)) //get only identification of the PDO mapping object
            {
                ...
                break;
							  case 0x3:   //pvm PDO selected
                    sDetectedModuleIdentList.aEntries[cnt] = PVM_MODULE_ID;
                break;
							  case 0x4:   //ptm PDO selected
                    sDetectedModuleIdentList.aEntries[cnt] = PTM_MODULE_ID;
                break;
        ...
        switch ((sRxPDOassign.aEntries[PDOAssignEntryCnt] & 0x000F))    //mask Axis type (supported modes)
        {
            ...
            case 2:
                ...
                break;
						...
						case 3:
                    /*drive mode supported    pvm(profile velocity mode) : bit 2*/
                LocalAxes[PDOAssignEntryCnt].Objects.objSupportedDriveModes= 0x4;
                ...
                break;
						...
						case 4:
                    /*drive mode supported    ptm(profile torque mode) : bit 3*/
                LocalAxes[PDOAssignEntryCnt].Objects.objSupportedDriveModes= 0x8;
                for(u16cnt =0 ; u16cnt < LocalAxes[PDOAssignEntryCnt].Objects.sRxPDOMap4.u16SubIndex0;u16cnt++)
                {
                    OutputSize += (UINT16)(LocalAxes[PDOAssignEntryCnt].Objects.sRxPDOMap4.aEntries[u16cnt] & 0xFF);;
                }
                break;
        }
    }
        ...
        for(PDOAssignEntryCnt = 0; PDOAssignEntryCnt < sTxPDOassign.u16SubIndex0; PDOAssignEntryCnt++)
        {
            ...
            case 2: /*csv*/
                ...
                break;
						...
						case 4: /*ptm*/
                for(u16cnt =0 ; u16cnt < LocalAxes[PDOAssignEntryCnt].Objects.sTxPDOMap4.u16SubIndex0;u16cnt++)
                {
                    InputSize +=(UINT16)(LocalAxes[PDOAssignEntryCnt].Objects.sTxPDOMap4.aEntries[u16cnt] & 0xFF);
                }
                break;
            }
        }
    ...
    for (j = 0; j < sTxPDOassign.u16SubIndex0; j++)
    {
        ...
        case 0: //copy csp/csv TxPDO entries
            {							
						  //*pTmpData++ = LocalAxes[AxisIndex].Objects.objStatusWord;
							*pTmpData++ = LocalAxes[AxisIndex].i16State;                           
        ...
        case 2://copy csv TxPDO entries
            ...
            break;
				...
				case 4://copy ptm TxPDO entries
            {							
							*pTmpData++ = LocalAxes[AxisIndex].Objects.objStatusWord;							
							*pTmpData++ = LocalAxes[AxisIndex].Objects.objTorqueActualValue;			
            }
            break;
        }
    }
    ...
    for (j = 0; j < sRxPDOassign.u16SubIndex0; j++)
    {
        ...
        case 2:    //csv RxPDO    entries
            ...
            break;
				...
				case 3:    //pvm RxPDO    entries
            {
							LocalAxes[AxisIndex].Objects.objControlWord = SWAPWORD(*pTmpData++);	
							LocalAxes[AxisIndex].Objects.objTargetVelocity =SWAPWORD(*pTmpData++);
							LocalAxes[AxisIndex].Objects.objTargetVelocity+=(SWAPWORD(*pTmpData++)<<16);
							LocalAxes[AxisIndex].Objects.ObjProfileAcceleration =SWAPWORD(*pTmpData++);
							LocalAxes[AxisIndex].Objects.ObjProfileAcceleration+=(SWAPWORD(*pTmpData++)<<16);	
							LocalAxes[AxisIndex].Objects.ObjProfileDeceleration =SWAPWORD(*pTmpData++);
							LocalAxes[AxisIndex].Objects.ObjProfileDeceleration+=(SWAPWORD(*pTmpData++)<<16);								
            }
            break;
						
				case 4:    //ptm RxPDO    entries
            {
							LocalAxes[AxisIndex].Objects.objControlWord = SWAPWORD(*pTmpData++);	
							LocalAxes[AxisIndex].Objects.objTargetTorque =SWAPWORD(*pTmpData++);
							LocalAxes[AxisIndex].Objects.objTorqueSlope =SWAPWORD(*pTmpData++);
							LocalAxes[AxisIndex].Objects.objTorqueSlope+=(SWAPWORD(*pTmpData++)<<16);							
            }
            break;
        }
    }
...
void APPL_Application(void)
{
    ...
    for(i = 0; i < MAX_AXES;i++)
    {
		...
		temp=LocalAxes[0].bAxisIsActive;
		temp=LocalAxes[0].fCurPosition;
		temp=LocalAxes[0].i16State;
		temp=LocalAxes[0].u32CycleTime;
		temp=LocalAxes[0].Objects.objControlWord;
		temp=LocalAxes[0].Objects.objErrorCode;
		temp=LocalAxes[0].Objects.objModesOfOperation;
		temp=LocalAxes[0].Objects.objModesOfOperationDisplay;
		temp=LocalAxes[0].Objects.objStatusWord;
		temp=LocalAxes[0].Objects.objTargetPosition;
		temp=LocalAxes[0].Objects.objTargetVelocity;
		temp=LocalAxes[0].Objects.objTargetTorque;
		temp=LocalAxes[0].Objects.ObjProfileAcceleration;
		temp=LocalAxes[0].Objects.ObjProfileDeceleration;
		temp=LocalAxes[0].Objects.objTorqueSlope;
		temp=LocalAxes[0].Objects.objPositionActualValue;
		temp=LocalAxes[0].Objects.objVelocityActualValue;
		temp=LocalAxes[0].Objects.objTorqueActualValue;
}
